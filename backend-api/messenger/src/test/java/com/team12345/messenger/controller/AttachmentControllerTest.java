...
import com.team12345.messenger.config.TestSecurityConfig;
...
public class AttachmentControllerTest {
    ...
    void uploadFile_ShouldReturnAttachmentInfo() throws Exception {
        ...
        mockMvc.perform(multipart("/attachments/upload")
                .file(file)
                .param("messageId", "1")
    ...
    void getAttachment_ShouldReturnInfo() throws Exception {
        ...
        mockMvc.perform(get("/attachments/1").with(authentication(auth)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1L));
    ...
    void getAttachmentsByMessage_ShouldReturnList() throws Exception {
        ...
        mockMvc.perform(get("/attachments/message/10").with(authentication(auth)))
                .andExpect(status().isOk())
    ...
    void deleteAttachment_ShouldReturnNoContent() throws Exception {
        ...
        mockMvc.perform(delete("/attachments/1").with(authentication(auth)))
